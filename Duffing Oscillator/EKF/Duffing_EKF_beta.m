...
% Defining the nonlinear process function
 f={@(x1,x2,x3,t) (x1+x2*t);
    @(x1,x2,x3,t) (x2+(-gamma*x2+omega^2*x1-x3*x1^3+AMP*cos(rho*t))*t)
    @(x1,x2,x3,t) (x3);
   }; 
    %Prediction step
 ...
 Aa = [0 1 0;omega^2-3*xbar(3)*xbar(1)^2 -gamma -xbar(1)^3;0 0 0];
...
legend({'Noisy angles', 'EKF Estimate'},'FontSize',7)
xlabel('Time (s)')
hl = ylabel('$\theta~(rad)$');
...
set(gca,'fontsize',10)
set(gca,'yticklabel',num2str(tix,'%.1f'))
legend({'True angular velocity', 'EKF Estimate'},'FontSize',7)
xlabel('Time (s)')
...
% plot(time,KalmanGain(1,:),'b','linewidth',2);
% xlabel('Time (s)')
% hl = ylabel('Gain in $\theta$');
% set(hl, 'Interpreter', 'latex');
% grid on;
% 
% subplot(3,1,2)
% plot(time,KalmanGain(2,:),'b','linewidth',2);
% xlabel('Time (s)')
% hl = ylabel('Gain in $\dot{\theta}$');
% set(hl, 'Interpreter', 'latex');
% grid on;
% 
% subplot(3,1,3)
% plot(time,KalmanGain(3,:),'b','linewidth',2);
% xlabel('Time (s)')
% hl = ylabel('Gain in $\beta$');
% set(hl, 'Interpreter', 'latex');
% grid on;
...
rmse_3 = sqrt(sum((xbarEstimate(3,:)-beta.*ones(1,length(time))).^2)./numel(xbarEstimate(2,:)))
% 
