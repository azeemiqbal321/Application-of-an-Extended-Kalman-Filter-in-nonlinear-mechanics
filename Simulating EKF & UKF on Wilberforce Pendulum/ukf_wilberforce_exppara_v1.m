...
%some parameters
Nk=4000; %Number of iterations
ndim=4;
n_meas=2; %we are only measuring the position with a displacement sensor

sensor_x_variance=0.001^2;
sensor_y_variance=0.001^2;
dt=0.01; %time step

Q_x_variance=0.001; %process noise variance, assume a piecewise constant white noise
Q=piecewise_white_noise(2,Q_x_variance,dt);
Q=kron(Q,eye(2));
...
x_initial_actual=[z; 0; epsilon; theta];  
x=[z; 0; 0;theta]; %this is how the Kalman filter is initialized
P_max=10;
P=P_max*eye(4);
...
f={@(x1,x2,x3,x4,t) (x1+x2*t);
   @(x1,x2,x3,x4,t) (x2+(-omega^2*x1-0.5/inertia*x4*x1)*t);
   @(x1,x2,x3,x4,t) (x3); %epsilon
   @(x1,x2,x3,x4,t) (x4); %theta
   };
...
for kk=1:2*ndim+1
ZZ(1,kk)=h{1}(y_sigma(1,kk),y_sigma(2,kk),y_sigma(4,kk));
ZZ(2,kk)=h{2}(y_sigma(3,kk),y_sigma(4,kk),y_sigma(4,kk));
end
...
zz(2)=model(k,2)+sqrt(sensor_y_variance)*randn; %this is the theta measurement
...
data(k,:)=[zz(1) zz(2) x(1) x(2) x(3) x(4) P(1,1) P(2,2) P(3,3) P(4,4)];
end
% 
...
rmse_1 = sqrt(mean(data(:,5)-epsilon.*ones(1,numel(data(:,5)))').^2)
% % %%%%%%%%%%
% % 
% % 
