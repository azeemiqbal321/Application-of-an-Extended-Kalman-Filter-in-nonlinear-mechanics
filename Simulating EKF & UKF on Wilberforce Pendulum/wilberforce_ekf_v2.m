...
variance_process=0.05^2; %process variance in position z
...
D = obsNoise_z;
 ...
 f={@(x1,x2,x3,x4,t) (x1+x2*t);
    @(x1,x2,x3,x4,t) (x2+(-omega^2*x1-0.5/mass*x4*x3)*t);
    @(x1,x2,x3,x4,t) (x3);
    @(x1,x2,x3,x4,t) (x4)}; 
...
for i = 2:length(time)
 ...
 A = [0 1 0 0;-omega^2 0 -0.5/mass*epsilon 0;0 0 0 1;-0.5/inertia*epsilon 0 -omega^2 0];
 F = eye(length(xbar))+h*A; %State transition matrix
 P = F*P*F' + Q;
 % Observation update
 K = (P*H')/(H*P*H'+D); % aka K = ...
