...
Q_x_variance=0.01; %process noise variance, assume a piecewise constant white noise
Q_y_variance=0.8;
...
x=[z; 0; theta; 0; 3]; %this is how the Kalman filter is initialized
P_max=1;
P=P_max*eye(5);
...
f={@(x1,x2,x3,x4,x5,t) (x1+x2*t);
   @(x1,x2,x3,x4,x5,t) (x2+(-omega^2*x1-0.5/mass*x5*x3)*t);
   @(x1,x2,x3,x4,x5,t) (x3+x4*t)
   @(x1,x2,x3,x4,x5,t) (x4+(-omega^2*x3-0.5/inertia*x5*x1)*t)
   @(x1,x2,x3,x4,x5,t) (x5)};
...
ZZ(2,kk)=h{2}(y_sigma(1,kk),y_sigma(2,kk),y_sigma(4,kk),y_sigma(5,kk));
end
...
% 
subplot(2,2,2)
plot(tt,data(:,2),'ro'); hold on;
plot(tt,data(:,6),'b-','linewidth',2); grid on;
dim = [.58 .80 .1 .1];
str = 'b)';
annotation('textbox',dim,'String',str,'FitBoxToText','on','LineStyle','none')
