...
%plotyy(t,measurements(:,1),t,measurements(:,2))
...
x=[z,0,theta,0,10,10,10]'; %this is how the Kalman filter is initialized
P_max=10;
P=P_max*eye(7);
...
ZZ=zeros(n_meas,2*ndim);
for kk=1:2*ndim
ZZ(1,kk)=h{1}(y_sigma(1,kk),y_sigma(2,kk),y_sigma(5,kk),y_sigma(7,kk));
ZZ(2,kk)=h{2}(y_sigma(3,kk),y_sigma(4,kk),y_sigma(6,kk),y_sigma(7,kk));
end
...
zz=[measurements(k,1)+sqrt(sensor_x_variance)*randn;
    measurements(k,3)+sqrt(sensor_y_variance)*randn];
...
data(k,:)=[zz(1) zz(2) x(1) x(3) x(5) x(6) x(7)];
end
