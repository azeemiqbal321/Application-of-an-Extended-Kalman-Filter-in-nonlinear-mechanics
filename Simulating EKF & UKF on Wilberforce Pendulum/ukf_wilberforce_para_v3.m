...
f={@(x1,x2,x3,x4,t) (x1+x2*t);
   @(x1,x2,x3,x4,t) (x2+(-omega^2*x1-0.5/inertia*x3*x1)*t);
...
ZZ(2,kk)=h{2}(y_sigma(3,kk),y_sigma(4,kk));
end
...
zz(2)=model(k,3)+sqrt(sensor_y_variance)*randn; %this is the theta measurement
...
legend('Measured displacement','Filtered','location','northeast');
xlabel('t'); ylabel('z (m)');
grid on; hold off;
% 
subplot(2,2,2)
...
grid on; 
suptitle('Unscented Kalman Filter Wilberforce Parameter Estimation')
% % %%%%%%%%%%
% % 
% % 
